...
import { demoCredentials, signIn, signUp } from '../auth';
    ...
    try {
      if (mode === 'signin') {
        ...
        const artisan: ArtisanProfile = {
          ...
          id: `artisan-${Date.now()}`,
          name: name.trim(),
          avatar: avatar || defaultArtisan.avatar,
          hindiName: name.trim(),
          story: hi
        ...
        <div className="p-6 sm:p-10 lg:p-12">
            ...
            {mode === 'signup' && (
              ...
              <div className="mt-4">
                <label className="block text-xs font-bold text-[#534439] mb-1.5">
                  {hi ? 'प्रोफ़ाइल फोटो' : 'Profile photo'}
                </label>
          ...
          {mode === 'signin' && (
            <button
              ...
              className="w-full mt-4 py-3 rounded-xl border border-[#D8C2B5] bg-white text-[#8E4E14] text-xs font-bold hover:bg-[#FAF9F6] transition-all"
            >
              {hi ? 'Demo Account भरें' : 'Use Demo Account'}
            </button>
          )}
