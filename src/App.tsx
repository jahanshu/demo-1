...
import { ensureDemoUser, getCurrentUser, getUserProductsKey, getLegacyProductsKey, signOut, updateCurrentUser } from './auth';
...
export function App() {
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      try {
        await ensureDemoUser(INITIAL_ARTISAN);
  ...
  const handleAuthenticated = ({ artisan: authenticatedArtisan }: { artisan: ArtisanProfile }) => {
    ...
    if (saved) {
      ...
      setProducts(user.id === 'user-demo' ? INITIAL_PRODUCTS : []);
    }
